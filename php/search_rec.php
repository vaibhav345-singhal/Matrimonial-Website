...
<div class="grid_3">
  ...
  <div class="col-md-9 profile_left1">
	  ...
	  <?php
			if(empty($_REQUEST["gender"]) || empty($_REQUEST["religion"])|| empty($_REQUEST["state"]) || empty($_REQUEST["city"]) || empty($_REQUEST["occup"]) ||empty($_REQUEST["marital"])){
				...
				$marital = mysqli_real_escape_string($conn,$_REQUEST["marital"]);
					...
					$rt = mysqli_query($conn,"SELECT count(*) FROM `users` WHERE 
					  gender='$gender' AND state = '$state' AND religion='$religion' AND marital='$marital' AND occupation='$occupation' AND city='$city' AND email!='$email'");
				...
				$rs = mysqli_query($conn,"SELECT * FROM `users`  where gender='$gender' AND religion='$religion' AND state='$state' AND city='$city' AND occupation='$occupation' AND marital='$marital' AND email!='$email' LIMIT $offset,$limit");
    ...
    <div class="profile_top"><a href="view_profile.php?id=<?=$r["code"]?>">
      <h2>Profile Id: <?=$r["code"]?></h2>
	    ...
	    <div class="col-sm-6">
		   ...
		   <div class="buttons">
			   <div class="vertical" a href="view_profile.php">View Profile</div>
		   </div>
	    </div>
	...
	<?php
	 ...
	 if($offset!=0){
	  ...
	  ?>
	  <a style="color: maroon" href="search_rec.php?page=<?=($page-1)?>&gender=<?=$gender?>&state=<?=$state?>&religion=<?=$religion?>&city=<?=$city?>&occup=<?=$occupation?>&marital=<?=$marital?>"><button  class="vertical">Previous</button>
	  <?php
		}
	  ?>
	...
	?>
	  ...
	  <a style="color: maroon" href="search_rec.php?page=<?=($page+1)?>&gender=<?=$gender?>&state=<?=$state?>&religion=<?=$religion?>&city=<?=$city?>&occup=<?=$occupation?>&marital=<?=$marital?>"><button  class="vertical">Next</button>
